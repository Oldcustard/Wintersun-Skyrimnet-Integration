...
- Deity knowledge pack upgraded to knowledge pack format version 3 (`format_version` 3 with per-entry `key` fields) — the version shipped in 3.1.0 used the old format 2, which Beta 25 rejects, so deity lore never loaded as world knowledge. The pack is regenerated by `build_knowledge_pack.py` and validated with the SkyrimNet devkit (`content-validate.exe`) before packaging.
